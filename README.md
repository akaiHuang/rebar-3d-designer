...
Rebar 3D Designer 是一套瀏覽器端的 3D 配筋設計與視覺化工具，以參數化方式建立鋼筋配置並即時檢視。適合結構工程設計溝通、教學展示與 WebGL 工程應用原型，將配筋資料變成可互動的 3D 模型。

## About (EN)

Rebar 3D Designer is a browser-based parametric reinforcement design and visualization tool for structural engineering. It helps teams present and validate rebar layouts through interactive 3D models.
